    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-background/95 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="p-4 rounded-lg bg-card/30 border border-border/40">
            ...
            <p className="text-sm font-semibold text-foreground">{
              burnInfo ? `Burned ${burnInfo.amount ?? '—'} ${burnInfo.symbol ?? ''}` : '—'
            }</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 pt-2">
          <a href={`https://explorer.solana.com/tx/${burnInfo?.txid}`} target="_blank" rel="noreferrer" className="w-full inline-flex py-3 px-4 rounded-lg bg-gradient-to-r from-accent to-accent/80 text-accent-foreground font-semibold hover:from-accent/90 hover:to-accent/70 smooth-transition shadow-lg shadow-accent/20 items-center justify-center gap-2">
